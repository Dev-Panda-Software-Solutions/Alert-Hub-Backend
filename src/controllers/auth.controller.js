...
const register = async (req, res, next) => {
  try {
    ...
    const user = await prisma.user.create({
      data: { name, email: email.toLowerCase(), passwordHash, country, plan: 'PERSONAL', trialEndsAt },
    });
    ...
    prisma.reminder.createMany({
      data: [
        { userId: user.id, title: 'Credit Card Bill', module: 'FINANCE', category: 'credit_card', amount: 5000, dueDate: seedDate(7),  recurrence: 'MONTHLY', schedule: [3, 7], channels: ['push', 'email'] },
        { userId: user.id, title: 'Electricity Bill',  module: 'FAMILY',  category: 'electricity',  amount: 2500, dueDate: seedDate(12), recurrence: 'MONTHLY', schedule: [3],    channels: ['push', 'email'] },
    ...
    res.status(201).json({
      token,
      user: { id: user.id, name: user.name, email: user.email, country: user.country, plan: user.plan, simBalance: user.simBalance, avatarUrl: user.avatarUrl, trialEndsAt: user.trialEndsAt, trialSeen: user.trialSeen },
    });
  } catch (err) { next(err); }
};
...
const sandbox = (req, res) => {
  ...
  res.json({
    token,
    user: { id: 'sandbox', name: 'Guest User', email: 'guest@sandbox.com', country: 'India', plan: 'FREE', simBalance: 75000, avatarUrl: null, sandbox: true, trialEndsAt: null, trialSeen: true },
  });
};
...
const me = (req, res) => {
  const u = req.user;
  res.json({ id: u.id, name: u.name, email: u.email, country: u.country, plan: u.plan, simBalance: u.simBalance, avatarUrl: u.avatarUrl || null, sandbox: u.sandbox || false, trialEndsAt: u.trialEndsAt || null, trialSeen: u.trialSeen ?? true });
};
...
const verifySignupOtp = async (req, res, next) => {
  try {
    ...
    const trialEndsAt = new Date(Date.now() + 28 * 24 * 60 * 60 * 1000);
    const user = await prisma.user.create({
      data: { name: entry.name, email: normalised, passwordHash: entry.passwordHash, country: entry.country, plan: 'PERSONAL', trialEndsAt },
    });
    signupPending.delete(normalised);
